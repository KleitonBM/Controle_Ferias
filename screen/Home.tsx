...
export default function HomeScreen({ employees, schedules, onOpenRegister, onOpenMonthly, onOpenSchedule, onDeleteSchedules }) {
    ...
    // Referências para sincronizar o scroll horizontal do cabeçalho com o da grade
    ...
    const getStatusEDatas = (startDateStr, durationDays, empId) => {
        ...
        const anchor = ANCHORS[emp.letra] || ANCHORS['A'];

        const getDayType = (date) => {
            const diff = Math.floor((date.getTime() - anchor.getTime()) / (1000 * 60 * 60 * 24));
            return ((diff % 8) + 8) % 8 < 4 ? 'TRABALHO' : 'FOLGA';
        };
    ...
    const getScaleStyle = (targetDate, empLetra = 'A') => {
        ...
        <SafeAreaView style={styles.container}>
            ...
            <Modal visible={!!editingSchedule && !showConfirmDelete} transparent animationType="slide">
                <TouchableOpacity style={styles.modalOverlay} onPress={() => setEditingSchedule(null)}>
                    <View style={styles.editModalContent}>
                                    ...
                                    return (
                                        <>
                                            <View style={styles.detailBox}>
                                                <Text style={[styles.infoLabel, {color: COLORS.PRIMARY}]}>SAÍDA (FOLGA)</Text>
                                                <Text style={[styles.infoValue, {color: COLORS.PRIMARY}]}>{info.saida}</Text>
                                            </View>
                                            <View style={styles.detailDivider} />
                                            <View style={styles.detailRow}>
                                                <View style={styles.detailItem}>
                                                    <Text style={styles.infoLabel}>INÍCIO</Text>
                                                    <Text style={styles.infoValue}>{editingSchedule.startDate}</Text>
                                                </View>
                                                <View style={styles.detailItem}>
                                                    <Text style={styles.infoLabel}>RETORNO</Text>
                                                    <Text style={[styles.infoValue, {color: COLORS.SAVE}]}>{info.retorno}</Text>
                                                </View>
                                            </View>
                                        </>
                                    );
                        ...
                        <TouchableOpacity style={[styles.editBtn, {backgroundColor: '#FF9800', marginBottom: 10}]} onPress={() => {
                        ...
                        <TouchableOpacity style={[styles.editBtn, {backgroundColor: COLORS.DELETE}]} onPress={() => setShowConfirmDelete(true)}>
                            <Text style={styles.btnT}>EXCLUIR</Text>
                        </TouchableOpacity>
                    </View>
                </TouchableOpacity>
            </Modal>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.confirmBox}>
                        ...
                        <View style={styles.confirmActions}>
                            <TouchableOpacity style={[styles.confirmBtn, {backgroundColor: COLORS.LINE}]} onPress={() => setShowConfirmDelete(false)}>
                                <Text>NÃO</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.confirmBtn, {backgroundColor: COLORS.DELETE}]} onPress={async () => {
                                if (onDeleteSchedules && editingSchedule) {
                                    await onDeleteSchedules([editingSchedule.id]);
                                }
                                setShowConfirmDelete(false); 
                                setEditingSchedule(null);
                            }}>
                                <Text style={styles.btnT}>SIM, EXCLUIR</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.mainHeader}>
                ...
                <Switch 
                    ...
                    onValueChange={setIsTimelineView}
                ...
                {isTimelineView ? (
                    <>
                        ...
                        <View style={styles.stickyHeaderContainer}>
                            {/* Canto Superior Esquerdo (Vazio ou Título) */}
                            <View style={styles.namesHeaderCorner}>
                                <Text style={{fontSize: 10, color: COLORS.MUTED, fontWeight: 'bold'}}>BALANCEIROS</Text>
                            </View>
                            
                            {/* Scroll Horizontal dos Dias (Controlado pelo scroll de baixo) */}
                            <ScrollView 
                                horizontal 
                                ref={headerScrollRef} 
                                scrollEnabled={false} // Desabilita toque direto, move apenas via código
                        ...
                        {/* --- 2. CORPO COM SCROLL VERTICAL (NOMES + GRADE) --- */}
                                ...
                                <View style={styles.namesColumn}>
                                    {employeesFiltered.map(item => (
                                        <View key={item.id} style={styles.nameRow}>
                                            ...
                                            <Text style={{fontSize: 8, color: COLORS.PRIMARY}}>Letra {item.letra}</Text>
                                        </View>
                                    ))}
                                </View>

                                {/* COLUNA DIREITA (GRADE) - Controla o Scroll do Header */}
                                <ScrollView 
                                    horizontal 
                                    ref={gridScrollRef}
                                    style={styles.gridScroll}
                                    onScroll={handleGridScroll}
                                    scrollEventThrottle={16} // Garante fluidez na sincronização
                                >
                                    <View>
                                        {employeesFiltered.map(emp => (
                                            <View key={emp.id} style={styles.row}>
                                                {/* Células de fundo */}
                                                {timelineDays.map(d => <View key={d.getTime()} style={[styles.cell, getScaleStyle(d, emp.letra)]} />)}
                                                ...
                                                {(schedules || []).filter(s => s.employeeId === emp.id).map(sch => {
                                                    ...
                                                    const effectiveEndIdx = endIdx === -1 ? timelineDays.length - 1 : endIdx;
                                                    const left = effectiveStartIdx * DAY_WIDTH;
                                                    const width = (effectiveEndIdx - effectiveStartIdx + 1) * DAY_WIDTH;

                                                    return (
                                                        <TouchableOpacity 
                                                            key={sch.id} 
                                                            onPress={() => setEditingSchedule(sch)} 
                                                            style={[styles.bar, { left: left, width: width, backgroundColor: COLORS.VACATION }]}
                                                        >
                                    ...
                                    <View style={styles.cardContent}>
                                        <View style={styles.cardHeader}>
                                            <View>
                                                ...
                                                <Text style={{fontSize: 10, color: COLORS.PRIMARY, fontWeight: 'bold'}}>Letra {emp.letra}</Text>
                                            </View>
...
const styles = StyleSheet.create({
    ...
    // NOVO: Container do Cabeçalho Fixo
    ...
    daysHeader: { flexDirection: 'row', height: 40, backgroundColor: COLORS.HEADER }, // Removida a borda inferior aqui pois o container já tem
    dayBox: { width: DAY_WIDTH, alignItems: 'center', justifyContent: 'center', borderRightWidth: 1, borderColor: COLORS.LINE },
    dayLabel: { fontSize: 10, fontWeight: 'bold', color: COLORS.TEXT }, // Fonte um pouco menor para caber DD/MM
